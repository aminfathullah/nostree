import { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { motion } from 'motion/react';
import { 
  ArrowRight, 
  Folder,
  BarChart3,
  Mail,
  ExternalLink
} from 'lucide-react';
import logo from '../assets/logo.png';

export default function HomePage() {
  const navigate = useNavigate();
  const { pubkey, status } = useAuth();
  const isAuthenticated = Boolean(pubkey && status === "authenticated");
  const [claimedSlug, setClaimedSlug] = useState("");

  const handleClaim = (e: React.FormEvent) => {
    e.preventDefault();
    const clean = claimedSlug.trim().toLowerCase().replace(/[^a-z0-9-]/g, "");
    if (!clean) {
      navigate('/admin');
      return;
    }
    navigate(`/admin?claim=${encodeURIComponent(clean)}`);
  };

  return (
    <main className="min-h-screen bg-canvas text-txt-main flex flex-col justify-between selection:bg-brand selection:text-brand-fg">
      <nav className="border-b border-border bg-canvas/80 backdrop-blur-xs">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <img src={logo} alt="Nostree Logo" className="w-7 h-7 object-contain" />
            <span className="font-bold text-base tracking-tight">Nostree</span>
          </Link>
          <div className="flex items-center gap-2">
            {isAuthenticated ? (
              <Link
                to="/admin"
                className="text-xs font-semibold px-3.5 py-1.5 rounded-lg bg-brand text-brand-fg hover:bg-brand-hover transition-colors"
              >
                Dashboard
              </Link>
            ) : (
              <Link
                to="/admin"
                className="text-xs font-semibold px-3.5 py-1.5 rounded-lg bg-brand text-brand-fg hover:bg-brand-hover transition-colors"
              >
                Buka Editor
              </Link>
            )}
          </div>
        </div>
      </nav>

      <div className="max-w-xl mx-auto px-4 py-8 sm:py-12 w-full flex flex-col items-center text-center my-auto">
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.25 }}
          className="w-full space-y-2 mb-6"
        >
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-txt-main">
            Direktori &amp; Tautan Kerja
          </h1>
          <p className="text-xs sm:text-sm text-txt-muted max-w-md mx-auto leading-relaxed">
            Satukan folder dokumen, dashboard tim, spreadsheet, dan kontak koordinasi dalam satu alamat profil ringkas.
          </p>
        </motion.div>

        {isAuthenticated ? (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.25, delay: 0.05 }}
            className="mb-8"
          >
            <Link
              to="/admin"
              className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand hover:bg-brand-hover text-brand-fg font-semibold rounded-xl text-sm transition-colors cursor-pointer"
            >
              <span>Buka Dashboard Kerja</span>
              <ArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
        ) : (
          <motion.form
            onSubmit={handleClaim}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.25, delay: 0.05 }}
            className="w-full mb-8"
          >
            <div className="flex items-center gap-2 p-1.5 bg-card border border-border focus-within:border-brand rounded-xl transition-colors shadow-xs">
              <div className="flex items-center px-3 text-txt-dim flex-1 min-w-0">
                <span className="text-xs sm:text-sm font-semibold text-txt-muted select-none">nostree.me/</span>
                <input
                  type="text"
                  value={claimedSlug}
                  onChange={(e) => setClaimedSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                  placeholder="nama-atau-tim"
                  className="w-full bg-transparent text-xs sm:text-sm font-medium text-txt-main placeholder:text-txt-dim focus:outline-none ml-1"
                />
              </div>
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-brand hover:bg-brand-hover text-brand-fg font-semibold rounded-lg text-xs transition-colors shrink-0 cursor-pointer"
              >
                <span>Buat Link</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
            <p className="text-[11px] text-txt-dim mt-2 text-center">
              Contoh: budi-santoso atau tim-data
            </p>
          </motion.form>
        )}

        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.25, delay: 0.1 }}
          className="w-full bg-card border border-border rounded-2xl p-4 sm:p-5 text-left shadow-xs"
        >
          <div className="flex items-center gap-3 pb-3.5 border-b border-border">
            <div className="w-9 h-9 rounded-xl border border-brand/20 bg-brand/10 text-brand font-bold flex items-center justify-center text-xs">
              BS
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h2 className="font-bold text-xs sm:text-sm text-txt-main">Budi Santoso</h2>
                <span className="text-[9px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded font-medium">
                  Aktif
                </span>
              </div>
              <p className="text-[11px] text-txt-dim">Divisi Analisis Data &amp; Pelaporan</p>
            </div>
          </div>

          <div className="space-y-2 pt-3">
            <div className="flex items-center justify-between p-2.5 rounded-lg bg-canvas/60 border border-border hover:border-brand/40 transition-colors">
              <div className="flex items-center gap-2.5">
                <div className="p-1.5 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400">
                  <Folder className="w-3.5 h-3.5" />
                </div>
                <div>
                  <p className="font-semibold text-xs text-txt-main">Folder Dokumen &amp; SOP Tim</p>
                  <p className="text-[10px] text-txt-dim">Google Drive</p>
                </div>
              </div>
              <ExternalLink className="w-3 h-3 text-txt-dim" />
            </div>

            <div className="flex items-center justify-between p-2.5 rounded-lg bg-canvas/60 border border-border hover:border-brand/40 transition-colors">
              <div className="flex items-center gap-2.5">
                <div className="p-1.5 rounded-md bg-indigo-500/10 text-indigo-600 dark:text-indigo-400">
                  <BarChart3 className="w-3.5 h-3.5" />
                </div>
                <div>
                  <p className="font-semibold text-xs text-txt-main">Dashboard Monitoring Capaian</p>
                  <p className="text-[10px] text-txt-dim">Metabase</p>
                </div>
              </div>
              <ExternalLink className="w-3 h-3 text-txt-dim" />
            </div>

            <div className="flex items-center justify-between p-2.5 rounded-lg bg-canvas/60 border border-border hover:border-brand/40 transition-colors">
              <div className="flex items-center gap-2.5">
                <div className="p-1.5 rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-400">
                  <Mail className="w-3.5 h-3.5" />
                </div>
                <div>
                  <p className="font-semibold text-xs text-txt-main">Kontak Koordinasi Pegawai</p>
                  <p className="text-[10px] text-txt-dim">Email &amp; Pesan Langsung</p>
                </div>
              </div>
              <ExternalLink className="w-3 h-3 text-txt-dim" />
            </div>
          </div>
        </motion.div>
      </div>

      <footer className="border-t border-border py-4 px-4 text-center text-[11px] text-txt-dim">
        <p>Nostree — Direktori &amp; Tautan Kerja Pegawai</p>
      </footer>
    </main>
  );
}
